...
public class LuceneIndexer {
	...
	public void indexParas(String indexDirPath, String parafilePath, boolean withEntity, boolean removeStops) throws IOException {
		...
		if(removeStops) {
			StreamSupport.stream(it.spliterator(), true).forEach(p->{
				try {
					this.indexParaRemoveStops(iw, p, i, withEntity);
				} catch (IOException e) {
	...
	private void indexParaRemoveStops(IndexWriter iw, Data.Paragraph para, int count, boolean withEntity) throws IOException {
		Document paradoc = new Document();
		String processedText = this.removeStopWords(para.getTextOnly().toLowerCase());
		paradoc.add(new StringField("paraid", para.getParaId(), Field.Store.YES));
		paradoc.add(new TextField("parabody", processedText, Field.Store.YES));
		if(withEntity) {
			String entityList = "";
			ArrayList<String> entities = (ArrayList<String>) para.getEntitiesOnly();
			for(String ent:entities)
